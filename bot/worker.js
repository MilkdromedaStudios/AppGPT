const DEFAULT_APP_URL = 'https://milkdromedastudios.github.io/AppGPT/';
const DEFAULT_GITHUB_URL = 'https://github.com/MilkdromedaStudios/AppGPT';
...
async function sendStart(chatId, user, env, payload = '') {
  ...
  const guideUrl = new URL('getting-started.html', getAppUrl(env)).href;
  ...
  const text = [
    ...
    '2. Describe the app you want, use a template, or attach a screenshot.',
    '3. Generate it. Your chat saves immediately, and the finished HTML stays attached to the session.',
    '',
...
async function sendHelp(chatId, env) {
  const text = [
    ...
    '',
    'AppGPT generates one complete Telegram-ready <code>index.html</code>. Chats are saved before model generation starts so you can return to them later.'
  ].join('\n');
  ...
  return telegram(env, 'sendMessage', {
    ...
    reply_markup: {
      inline_keyboard: [
        ...
        [{ text: '📖 Full Quick Start', url: new URL('getting-started.html', getAppUrl(env)).href }]
      ]
    }
  });
}
...
async function sendSection(chatId, env, view) {
  const labels = {
    build: ['Open AppGPT', 'Launch the builder and create a new Telegram Mini App.'],
    templates: ['Browse Templates', 'Start from AppGPT’s template library, then customize the prompt before generating.'],
